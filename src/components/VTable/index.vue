  ...
  <div class="v-table">
    <div class="v-header" :style="getHeaderWidth">
      <div v-for="(item, index) in option.columns" :key="index" class="v-header-item"
        ...
        <svg-icon :name="getSortImg({ item, index })" width="20px" height="20px" @click="onSortClick({ item, index })"
...
const sortImgMap = {
  null: 'icon_sequence_down',
  true: 'icon_sequence_nor',
  false: 'icon_sequence_up',
}
  ...
  switch (fontSize) {
    case "small":
      document
        ...
        .style.setProperty("--tableFontSize", "12px");
      document
    ...
    case "middle":
      document
        .querySelector(".v-header")
        .style.setProperty("--tableFontSize", "14px");
      document
        .querySelector(".scroll-content")
        .style.setProperty("--tableFontSize", "14px");
      break;
    case "big":
      document
        .querySelector(".v-header")
        .style.setProperty("--tableFontSize", "16px");
      document
        .querySelector(".scroll-content")
        .style.setProperty("--tableFontSize", "16px");
      break;
 ...
 * 根据排序状态返回icon
 */
function getSortImg({ item, index }) {
  switch (item.asc) {
    case 'undefinde':
      return 'icon_sequence_nor'
    case true:
      return 'icon_sequence_down'
    case false:
      return 'icon_sequence_up'
    default:
      return 'icon_sequence_nor'
  }
}
...
function getHeaderWidth() {
  let widthArray =
    ...
    props.option.columns.length &&
    props.option.columns.map((item) => {
...
function onSortClick({ item, index }) {
  props.option.columns.map((item, i) => {
    if (i === index) {
